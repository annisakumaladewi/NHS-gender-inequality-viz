...
p <- ggplot(gpg_long, aes(x = year_end, y = gap_pct, group = metric, color = metric)) +
  ...
  # Titles / caption
  labs(
    title = "Gender Pay Gap at NHS Barts Trust 2018–2021",
...
Labelling
```{r}
# Inline labels (manual placement = cleaner than ggrepel for 2 labels)
bonus_labels <- tibble(
  year_end = c(2020.5, 2020.5),
...
p_barbell_v <- ggplot(bonus_df, aes(x = year_end)) +
  ...
  theme(
    ...
    # margins: tight so it can sit beside another chart
    plot.margin = margin(t = 8, r = 10, b = 10, l = 0)
  )
...
# Stack order
# Put Unknown on top (often a better convention)
